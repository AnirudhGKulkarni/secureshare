...
type UserRecord = {
  ...
  lastName?: string;
  email?: string;
  role?: "admin" | "client" | "super_admin" | "superadmin";
...
export function OverviewSection() {
  ...
  const [recentRequests, setRecentRequests] = useState<
    Array<{ name: string; company?: string | null; status: "pending" | "approved" | "rejected"; createdAt?: any }>
  >([]);
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <Card
            key={i}
            className="overflow-hidden transition-all hover:shadow-xl border-0"
            style={{
              background: `linear-gradient(135deg, ${["#113738", "#0d5a5f", "#0a7c87", "#0a9db0"][i]} 0%, ${["#0d5a5f", "#0a7c87", "#0a9db0", "#1a9fb5"][i]} 100%)`,
            }}
          >
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-white/80">{stat.name}</CardTitle>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">Monthly growth of admins and clients across the platform</p>
          </CardHeader>
          <CardContent>
                ...
                {recentRequests.map((request, i) => (
                  <div key={i} className="flex items-center gap-4 p-3 rounded-lg bg-secondary/50 border">
                    ...
                    <div className="text-right">
                      <span
                        className={`text-xs px-2 py-1 rounded ${request.status === "pending" ? "bg-orange-100 text-orange-700" : "bg-green-100 text-green-700"}`}
                      >
                        {request.status}
                      </span>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                {[
                  { name: "Database", desc: "Operational" },
                  { name: "Authentication", desc: "Operational" },
                  { name: "Storage", desc: "Operational" },
                ].map((s) => (
